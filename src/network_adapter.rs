...
mod v1 {
    ...
    pub fn from_string(csv:String)->Vec<NetWorkAdapter>{
            let mut rdr = Reader::from_reader(csv.as_bytes());
            let iter = rdr.deserialize();
            iter.filter_map(|x|x.ok()).collect()
    }

    pub fn get_networkadapters_with_csv_format()->Result<String,String>{
...
mod tests {
    ...
    fn test_network_adapter_from_string() {
        ...
        assert_eq!(from_string(csv.to_string()),
        vec![
            NetWorkAdapter{index:0,name:"Microsoft Kernel Debug Network Adapter".to_string(),net_enabled:None},
            NetWorkAdapter{index:14,name:"Qualcomm Atheros AR9485 Wireless Network Adapter".to_string(),net_enabled:Some(true)},
            NetWorkAdapter{index:16,name:"Qualcomm Atheros AR8171/8175 PCI-E Gigabit Ethernet Controller (NDIS 6.30)".to_string(),net_enabled:Some(false)},        
        ]);
    }
}
